...
public class FogVolumeCreator : Editor
{
    ...
    {
        var FogVolumeGO = new GameObject();

        //Icon stuff
        var Icon = Resources.Load("FogVolumeIcon") as Texture2D;
        ...
        EditorGUIUtility.SetIconForObject(FogVolumeGO, Icon);

        FogVolumeGO.name = "Fog Volume";
        FogVolumeGO.AddComponent<MeshRenderer>();
        FogVolumeGO.AddComponent<FogVolume>();
        FogVolumeGO.GetComponent<Renderer>().shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
        FogVolumeGO.GetComponent<Renderer>().receiveShadows = false;
        FogVolumeGO.GetComponent<Renderer>().reflectionProbeUsage =
                UnityEngine.Rendering.ReflectionProbeUsage.Off;
        FogVolumeGO.GetComponent<Renderer>().lightProbeUsage = UnityEngine.Rendering.LightProbeUsage.Off;
        Selection.activeObject = FogVolumeGO;
        if (SceneView.currentDrawingSceneView)
        {
            SceneView.currentDrawingSceneView.MoveToView(FogVolumeGO.transform);
        }
    }
