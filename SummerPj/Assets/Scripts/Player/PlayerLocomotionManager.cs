...
public class PlayerLocomotionManager : MonoBehaviour
{
    ...
    public void HandleMovement(float delta)
    {
        ...
        {
            _PlayerAnimationManager.UpdateAnimatorValues(_inputHandler._moveAmount, 0, _playerManager._isSprinting); // 애니메이션에게도 정보를 넘김
        }
    }
    ...
    public void HandleFalling(float delta, Vector3 moveDirection) // moveDirection : 떨어질때 이동할 방향
    {
        ...
        if (Physics.Raycast(origin, -Vector3.up, out hit, _minimumDistanceNeededToBeginFall, _ignoreForGroundCheck))
        {
            ...
            if (_playerManager._isInAir)
            {
                ...
                {
                    _PlayerAnimationManager.PlayTargetAnimation("Locomotion", false);
                    _inAirTimer = 0;
                }
        ...
        else
        {
            ...
            if (!_playerManager._isInAir)
            {
                ...
                {
                    _PlayerAnimationManager.PlayTargetAnimation("Falling", true); // 낙하 애니메이션
                }
